...
class MesNet(torch.nn.Module):
        def __init__(self):
            ...
            self.beta_measurement = 3*torch.ones(2).half()
            self.tanh = torch.nn.Tanh()
...
class TORCHIEKF(torch.nn.Module, NUMPYIEKF):
    ...
    Id1 = torch.eye(1).half()
    Id2 = torch.eye(2).half()
    Id3 = torch.eye(3).half()
    Id6 = torch.eye(6).half()
    IdP = torch.eye(21).half()
    ...
    def __init__(self, parameter_class=None):
        ...
        self.IdP = torch.eye(self.P_dim).half()
    ...
    def set_param_attr(self):
        ...
        self.cov0_measurement = torch.Tensor([self.cov_lat, self.cov_up]).half()
    ...
    def init_saved_state(self, dt, N, ang0):
        ...
        Rot_c_i[0] = torch.eye(3).half()
        return Rot, v, p, b_omega, b_acc, Rot_c_i, t_c_i

    def propagate(self, Rot_prev, v_prev, p_prev, b_omega_prev, b_acc_prev, Rot_c_i_prev, t_c_i_prev,
                  P_prev, u, dt):
        Rot_prev = Rot_prev.clone().half()
        acc_b = (u[3:6] - b_acc_prev).half()
        acc = (Rot_prev.mv(acc_b) + self.g).half()
        v = (v_prev + acc * dt).half()
        p = (p_prev + v_prev.clone() * dt + 1/2 * acc * dt**2).half()

        omega = (u[:3] - b_omega_prev)*dt.half()
        Rot = Rot_prev.mm(self.so3exp(omega)).half()

        b_omega = b_omega_prev.half()
        b_acc = b_acc_prev.half()
        Rot_c_i = Rot_c_i_prev.clone().half()
        t_c_i = t_c_i_prev.half()
    ...
    def propagate_cov(self, P, Rot_prev, v_prev, p_prev, b_omega_prev, b_acc_prev, u,
        ...
        F = P.new_zeros(self.P_dim, self.P_dim).half()
        G = P.new_zeros(self.P_dim, self.Q.shape[0]).half()
        Q = self.Q.clone().half()
        F[3:6, :3] = self.skew(self.g).half()
        F[6:9, 3:6] = self.Id3.half()
        G[3:6, 3:6] = Rot_prev.half()
        F[3:6, 12:15] = -Rot_prev.half()
        v_skew_rot = self.skew(v_prev).mm(Rot_prev).half()
        p_skew_rot = self.skew(p_prev).mm(Rot_prev).half()
        G[:3, :3] = Rot_prev.half()
        G[3:6, :3] = v_skew_rot.half()
        G[6:9, :3] = p_skew_rot.half()
        F[:3, 9:12] = -Rot_prev.half()
        F[3:6, 9:12] = -v_skew_rot.half()
        F[6:9, 9:12] = -p_skew_rot.half()
        G[9:12, 6:9] = self.Id3.half()
        G[12:15, 9:12] = self.Id3.half()
        G[15:18, 12:15] = self.Id3.half()
        G[18:21, 15:18] = self.Id3.half()
    ...
    def update(self, Rot, v, p, b_omega, b_acc, Rot_c_i, t_c_i, P, u, i, measurement_cov):
        ...
        Rot_body = Rot.mm(Rot_c_i).half()
        # velocity in imu frame
        v_imu = Rot.t().mv(v).half()
        omega = (u[:3] - b_omega).half()
        # velocity in body frame
        v_body = (Rot_c_i.t().mv(v_imu) + self.skew(t_c_i).mv(omega)).half()
        Omega = self.skew(omega).half()
        # Jacobian in car frame
        H_v_imu = Rot_c_i.t().mm(self.skew(v_imu)).half()
        H_t_c_i = self.skew(t_c_i).half()

        H = P.new_zeros(2, self.P_dim).half()
        H[:, 3:6] = Rot_body.t()[1:].half()
        H[:, 15:18] = H_v_imu[1:].half()
        H[:, 9:12] = H_t_c_i[1:].half()
        H[:, 18:21] = -Omega[1:].half()
        r = - v_body[1:].half()
        R = torch.diag(measurement_cov).half()
    ...
    def state_and_cov_update(Rot, v, p, b_omega, b_acc, Rot_c_i, t_c_i, P, H, r, R):
        ...
        Kt = torch.torch.linalg.solve(S, P.half().mm(H.half().t()).t()).half()
        ...
        v_up = (dR.mv(v) + dv).half()
        p_up = (dR.mv(p) + dp).half()


        b_omega_up = (b_omega + dx[9:12]).half()
        b_acc_up = (b_acc + dx[12:15]).half()

        dR = TORCHIEKF.so3exp(dx[15:18]).half()
        Rot_c_i_up = dR.mm(Rot_c_i).half()
        t_c_i_up = (t_c_i + dx[18:21]).half()

        I_KH = (TORCHIEKF.IdP - K.mm(H)).half()
        P_upprev = (I_KH.mm(P).mm(I_KH.t()) + K.mm(R).mm(K.t())).half()
    ...
    def rot_from_2_vectors(v1, v2):
        ...
        v1 = v1/torch.norm(v1).half()
        v2 = v2/torch.norm(v2).half()
        v = torch.cross(v1, v2).half()
        cosang = v1.matmul(v2).half()
        sinang = torch.norm(v).half()
        Rot = (TORCHIEKF.Id3 + TORCHIEKF.skew(v) + \
              TORCHIEKF.skew(v).mm(TORCHIEKF.skew(v))*(1-cosang)/(sinang**2)).half()
        return Rot

    @staticmethod
    def sen3exp(xi):
        phi = xi[:3].half()
        angle = torch.norm(phi).half()
        ...
        if isclose(angle, 0.):
            skew_phi = torch.Tensor([[0, -phi[2], phi[1]],
                          [phi[2], 0, -phi[0]],
                          [-phi[1], phi[0], 0]]).half()
            J = (TORCHIEKF.Id3 + 0.5 * skew_phi).half()
            Rot = (TORCHIEKF.Id3 + skew_phi).half()
        else:
            axis = (phi / angle).half()
            skew_axis = torch.Tensor([[0, -axis[2], axis[1]],
                              [axis[2], 0, -axis[0]],
                              [-axis[1], axis[0], 0]]).half()
            s = torch.sin(angle).half()
            c = torch.cos(angle).half()

            J = ((s / angle) * TORCHIEKF.Id3 + (1 - s / angle) * TORCHIEKF.outer(axis, axis)\
                   + ((1 - c) / angle) * skew_axis).half()
            Rot = (c * TORCHIEKF.Id3 + (1 - c) * TORCHIEKF.outer(axis, axis) \
                 + s * skew_axis).half()

        x = J.mm(xi[3:].view(-1, 3).t()).half()
        return Rot, x
    ...
    def to_rpy(Rot):
        ...
        pitch = torch.atan2(-Rot[2, 0], torch.sqrt(Rot[0, 0]**2 + Rot[1, 0]**2)).half()

        if isclose(pitch, np.pi / 2.):
            yaw = pitch.new_zeros(1).half()
            roll = torch.atan2(Rot[0, 1], Rot[1, 1]).half()
        elif isclose(pitch, -np.pi / 2.):
            yaw = pitch.new_zeros(1).half()
            roll = -torch.atan2(Rot[0, 1],  Rot[1, 1]).half()
        else:
            sec_pitch = 1. / pitch.cos()
            yaw = torch.atan2(Rot[1, 0] * sec_pitch, Rot[0, 0] * sec_pitch).half()
            roll = torch.atan2(Rot[2, 1] * sec_pitch, Rot[2, 2] * sec_pitch).half()
    ...
    def from_rpy(roll, pitch, yaw):
        ...
        return TORCHIEKF.rotz(yaw).mm(TORCHIEKF.roty(pitch).mm(TORCHIEKF.rotx(roll))).half()
    ...
    def normalize_rot(rot):
        ...
        U, _, V = torch.svd(rot).half()
        S = torch.eye(3).half()
        S[2, 2] = torch.det(U) * torch.det(V).half()
        return U.mm(S).mm(V.t()).half()

    def forward_nets(self, u):
        u_n = self.normalize_u(u).t().unsqueeze(0).half()
        u_n = u_n[:, :6].half()
        measurements_covs = self.mes_net(u_n, self).half()
        return measurements_covs

    def normalize_u(self, u):
        return ((u-self.u_loc)/self.u_std).half()

    def get_normalize_u(self, dataset):
        self.u_loc = dataset.normalize_factors['u_loc'].half()
        self.u_std = dataset.normalize_factors['u_std'].half()
    ...
    def set_Q(self):
        ...
        self.Q = torch.zeros(self.Q.shape[0], self.Q.shape[0]).half()
        self.Q[:3, :3] = self.cov_omega*beta[0]*self.Id3.half()
        self.Q[3:6, 3:6] = self.cov_acc*beta[1]*self.Id3.half()
        self.Q[6:9, 6:9] = self.cov_b_omega*beta[2]*self.Id3.half()
        self.Q[9:12, 9:12] = self.cov_b_acc*beta[3]*self.Id3.half()
        self.Q[12:15, 12:15] = self.cov_Rot_c_i*beta[4]*self.Id3.half()
        self.Q[15:18, 15:18] = self.cov_t_c_i*beta[5]*self.Id3.half()
...
def isclose(mat1, mat2, tol=1e-10):
    return (mat1 - mat2).abs().lt(tol).half()
...
def prepare_filter(args, dataset):
    ...
    if type(torch_iekf.g).__module__ == np.__name__:
        torch_iekf.g = torch.from_numpy(torch_iekf.g).half()
